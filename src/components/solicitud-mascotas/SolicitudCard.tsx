...
import { FaCheck, FaHeart, FaReply, FaTimes } from "react-icons/fa";

interface SolicitudCardProps {
  solicitud: INotificacionesInteresadosWithRelations;
  onResponse: (solicitud: INotificacionesInteresadosWithRelations) => void;
  onApprove: (solicitud: INotificacionesInteresadosWithRelations) => void;
  onReject: (solicitud: INotificacionesInteresadosWithRelations) => void;
}
...
export const SolicitudCard = ({
  ...
  const usuario = solicitud.usuario_envia;
            ...
            <div className="flex-1 min-w-0 mb-4 md:mb-0">
                    ...
                    <span className="text-sm font-medium text-gray-800 block">
                      ...
                      {usuario?.numero_documento && (
                        <span className="text-xs text-gray-600 font-normal ml-2">
                          (CC: {usuario.numero_documento}
                          {usuario?.dv ? `-${usuario.dv}` : ""})
                        </span>
                      )}
                    </span>
              ...
              <div className="flex flex-wrap gap-2 mt-2 text-sm">
                ...
                <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800">
                  {solicitud.visto ? "Visto" : "No visto"}
                </span>
                {solicitud.aprobado !== null && (
                  ...
                  >
                    {solicitud.aprobado ? "Aprobado" : "Rechazado"}
                  </span>
                )}
              </div>
